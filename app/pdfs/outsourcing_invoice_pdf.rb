class OutsourcingInvoicePDF
  ...
  def self.create purchase_data_current
    ...
    for num in 1..2 do
      ...
      report.page.item(:post).value(company.post)
      report.page.item(:company_name).value(company.name)
      report.page.item(:address).value(company.address)
      report.page.item(:address2).value(company.address2)
      #
      
      #外注先の情報取得
      supplier_master_id = purchase_data_current.supplier_id
      ...
      set_supplier_info(supplier_master_id)
  ...
  #仕入先(外注)の各種情報を拾う
  def set_supplier_info(supplier_master_id)
    #初期化
    @supplier_post = ""
    @supplier_address = ""
    @supplier_name = ""
    ...
    supplier = SupplierMaster.where(:id => supplier_master_id).first
  ...
  #仕入先(外注)の各種情報を拾う
  #adusu仕様--(現在未使用)
  def set_supplier_info_old(staff_id)
    #初期化
    @supplier_post = ""
    @supplier_address = ""
    @supplier_name = ""
